...
function convertToRomanNumerals(num) {
  ...
  if (fractPart === 5) {
    ...
    romanNum += 'IX';
  } else if (fractPart >= 1 && fractPart < 4) {
    while (fractPart !== 0) {
      romanNum += 'I';
      fractPart -= 1;
    }
  } else if (fractPart > 5 && fractPart < 9) {
    romanNum += 'V';
    while (fractPart !== 5) {
      romanNum += 'I';
      fractPart -= 1;
...
function convertNumberToString(numberStr) {
  ...
  let newStringTrimmed = '';
  for (let i = 0; i < newString.length - 1; i += 1) {
    newStringTrimmed += newString[i];
  }
  return newStringTrimmed;
}
